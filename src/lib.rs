...
impl<T: Hash + Copy> RelationalCore<T> {
    ...
    fn score(a: NodeScore, s: NodeScore, t: f64) -> f64 {
        ...
        let source_score =
            (a.source - (s.source / t)).max(0.).powi(2) * (t.powi(2) / (s.source * (t - 1.)));
        let dest_score =
            (a.dest - (s.dest / t)).max(0.).powi(2) * (t.powi(2) / (s.dest * (t - 1.)));
        let comb_score =
            (a.combined - (s.combined / t)).max(0.).powi(2) * (t.powi(2) / (s.combined * (t - 1.)));
